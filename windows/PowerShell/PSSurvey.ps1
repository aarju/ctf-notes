...
Function Get-HostInfo {
    $querydate = (Get-Date -Format s)
    $enclosureinfo = gwmi win32_systemenclosure
    $hostname = $enclosureinfo.PSComputerName
    $serialnumber = $enclosureinfo.SerialNumber
    $Manufacturer = $enclosureinfo.Manufacturer
    $OSInfo = Get-WmiObject -Class Win32_OperatingSystem
    $operatingsystem = $OSInfo.Caption
    $servicepack = $OSInfo.CSDVersion
    $computersystem = gwmi win32_computersystem
    $domain = $computersystem.Domain
    $hostinfo = "Hostname=$hostname;QueryDate=$querydate;QueryType=HostInfo;Operating System=$operatingsystem;Service Pack=$servicepack;SerialNumber=$serialnumber;Manufacturer=$Manufacturer;Domain=$domain"
    return $hostinfo
} 

Function Get-LoggedInUsers {
    $loggedinusers = gwmi -Class win32_computersystem | select username
    foreach ($user in $loggedinusers) {
        $querydate = (Get-Date -Format s)
        "Hostname=$ENV:COMPUTERNAME;QueryDate=$querydate;QueryType:LoggedInUsers;Logged In User=$($user.username)"
    }
}
...
Function Get-ScheduledTasks {
    <#
        .Synopsis
        Connects to the remote computer's scheduler service, parses through the tasks
        returns as a CSV
    #>
    #region Functions
    ...
    foreach ($Folder in $AllFolders) {
        if (($Tasks = $Folder.GetTasks(1))) {
            foreach ($task in $tasks) {
                # Create the csv object to return
                $querydate = (Get-Date -Format s)
                $hostname = $env:COMPUTERNAME
                $taskname = $task.name
                $author =  ([xml]$task.xml).Task.RegistrationInfo.Author
                $description = ([xml]$task.xml).Task.RegistrationInfo.Description
                $action = ([xml]$task.xml).Task.Actions.Exec.Command + ([xml]$task.xml).Task.Actions.Exec.Arguments
                $triggers = Get-TaskTrigger -Task $task                
                ...
                $taskstate = switch ($task.State) {
                        ...
                        4 {'Running'}
                        Default {'Unknown'}}
                "Hostname=$hostname;QueryDate=$querydate;QueryType=ScheduledTasks;TaskName=$taskname;Author=$author;"`
                + "Description=$description;TaskAction=$action;TaskTrigger=$triggers;TaskEnabled=$enabled;"`
                + "TaskLastRuntime=$lastruntime;TaskNextRuntime=$NextRunTime;TaskState=$taskstate"
            }
        }
    }
} #
...
Function Get-NetworkInfo {
    # Collect networking information about the system
    # interfaces, status, ip-addresses,MACAddress
    # Return a manually created csv with each line containing the following:
    # PSComputerName,MACAddress,IPAddress,Interface caption, Interface status
    $interfaces = [System.Net.NetworkInformation.NetworkInterface]::GetAllNetworkInterfaces()
    foreach ($interface in $interfaces){
        if (($interface.NetworkInterfaceType -ne "Ethernet") -and ($interface.NetworkInterfaceType -ne "Loopback")){
            Write-Output "!!! ALERT !!! $env:COMPUTERNAME has a network card that is not Ethernet: $($interface.Name), $($interface.Description)"
        } 
    }
    $adapterstatus = Get-WmiObject -Class Win32_NetworkAdapterConfiguration 
    $networkinfo = @()
    foreach ($adapter in $adapterstatus) {
        # check each of the physical interfaces to see if it has a MAC address
        if ($adapter.MACAddress) {
            # check to see if it has an assigned IP address
            if ($adapter.IPAddress){
                foreach ($ip in $adapter.IPAddress){ #enumerate through the IP addresses
                    $querydate = (Get-Date -Format s)
                    $hostname = $adapter.PSComputerName
                    $ipaddress = $adapter.IPAddress
                    $macaddress = $adapter.MACAddress
                    $description = $adapter.Description
                    $enabled = $adapter.IPEnabled
                    $networkinfo += "Hostname=$hostname;QueryDate=$querydate;QueryType=NetworkInfo;IPAddress=$ipaddress;MACAddress=$macaddress;InterfaceCaption=$description;InterfaceEnabled=$enabled"
                } 
            } else { # MAC Address but no IP Address
                $querydate = (Get-Date -Format s)
                $hostname = $adapter.PSComputerName
                $macaddress = $adapter.MACAddress
                $description = $adapter.Description
                $enabled = $adapter.IPEnabled
                $networkinfo += "Hostname=$hostname;QueryDate=$querydate;QueryType=NetworkInfo;MACAddress=$macaddress;InterfaceCaption=$description;InterfaceEnabled=$enabled"
            }
        }
    }    
    return $networkinfo
...
function Get-WMIPersist {
        ...
        IP address or hostname of the machine to query
    #>
    ...
    foreach ($binding in $bindings){
        ...
        #$consumer is the consumer bound to the event that gets executed

        # find the matching Filter and consumer
        $eventFilter = Get-WMIObject -Namespace root\Subscription -Class __EventFilter  | where {($_.Name).contains($binding.Filter.Split('"')[1])}
        $consumer = Get-WMIObject -Namespace root\Subscription -Class __EventConsumer | where {($_.Name).contains($binding.Consumer.Split('"')[1])}

        $hostname = $eventFilter.PSComputername
        $querydate = (Get-Date -Format s)
        $eventFilterName = $eventFilter.Name
        $eventFilterQuery = $eventFilter.Query
        $EventConsumerName = $consumer.Name
        $EventConsumerClass=$consumer.__CLASS
        $EventConsumerSourceName=$consumer.SourceName
        $EventConsumerExecutablePath=$consumer.ExecutablePath
        $EventConsumerScriptFileName=$consumer.ScriptFileName
        $EventConsumerScriptingEngine=$consumer.ScriptingEngine
        $EventConsumerScriptText=$consumer.ScriptText
        $EventConsumerCommandLineTemplate=$consumer.CommandLineTemplate
        $EventConsumerWorkingDirectory=$consumer.WorkingDirectory
        # long string for returning it all as a line in a csv
        "Hostname=$hostname;QueryDate=$querydate;QueryType=WMIPersistence;EventFilterName=$eventFilter;eventFilterQuery=$eventFilterQuery"`
            + ";EventConsumerName=$EventConsumerName;EventConsumerClass=$EventConsumerClass"`
            + ";EventConsumerSourceName=$EventConsumerSourceName;EventConsumerExecutablePath=$EventConsumerExecutablePath"`
            + ";EventConsumerScriptFileName=$EventConsumerScriptFileName;EventConsumerScriptingEngine=$EventConsumerScriptingEngine"`
            + ";EventConsumerScriptText=$EventConsumerScriptText;EventConsumerCommandLineTemplate=$EventConsumerCommandLineTemplate"`
            + ";EventConsumerWorkingDirectory=$EventConsumerWorkingDirectory"
    }
} #

Function Get-Services {
    # only return the running services
    $services = gwmi win32_service -Filter 'state="Running"'
    foreach ($service in $services){
        $querydate = (Get-Date -Format s)
        $hostname = $service.PSComputerName
        $servicename = $service.Name
        $servicepath = $service.PathName
        $startmode = $service.StartMode
        $caption = $service.DisplayName  
        "Hostname=$hostname;QueryDate=$querydate;QueryType=ServiceList;ServiceName=$servicename;ServicePath=$servicepath;StartMode=$startmode;Caption=$caption"
    }
} #

Function Get-InstalledPrograms {
    $colSoftware = Get-WmiObject -Class Win32Reg_AddRemovePrograms 
    foreach ($colItem in $colSoftware){
        $querydate = (Get-Date -Format s)
        $hostname = $colItem.PSComputerName
        $program = $colItem.DisplayName
        $publisher = $colItem.Publisher
        $version = $colItem.Version
        $InstallDate = $colItem.InstallDate
        $returnString += "Hostname=$hostname;QueryDate=$querydate;QueryType=InstalledPrograms;Program=$program;Version=$version`r`n"
    }
    if (-not $returnString.Contains("McAfee Host")){
        $returnString += "!!! ALERT !!! No HBSS Found on $env:COMPUTERNAME"      
    } 
    $returnString
} #
...
Function Get-Processes {
    $proccsv = @()
    $procs = (gwmi win32_process | select PSComputerName,Name,ProcessID,ParentProcessId,CommandLine)
    foreach ($proc in $procs){
        $querydate = (Get-Date -Format s)
        $hostname = $proc.PSComputerName
        $procname = $proc.name
        $procpid = $proc.ProcessID
        $procppid = $proc.ParentProcessId
        $proccmdline = $proc.CommandLine
        "Hostname=$hostname;QueryDate=$querydate;QueryType=ProcessList;ProcessName=$procname;ProcessID=$procpid;ParentProcessID=$procppid;Commandline=$proccmdline"
    }
} #

function Get-NetworkStatistics { 
    $properties = 'HostName',‘Protocol’,’LocalAddress’,’LocalPort’ 
    $properties += ‘RemoteAddress’,’RemotePort’,’State’,’ProcessName’,’PID’

    $results = netstat -ano | Select-String -Pattern ‘\s+(TCP|UDP)’ | ForEach-Object {
    ...
    foreach ($result in $results) {
        "Hostname=$($result.hostname);QueryDate=$querydate;QueryType=NetStat;Protocol=$($result.Protocol);ProcessName=$($result.ProcessName)" `
        + ";LocalAddress=$($result.LocalAddress);LocalPort=$($result.LocalPort);RemoteAddress=$($result.RemoteAddress)" `
        + ";RemotePort=$($result.RemotePort);State=$($result.State);PID=$($result.PID)"
    }
} #
...
Function get-DirtyBootTimes {
    $dirtyreboots = Get-WinEvent -FilterHashtable @{logname='System'; id=6008} -ErrorAction SilentlyContinue
    $querydate = (Get-Date -Format s)
    foreach ($log in $dirtyreboots){
        $rebootDate = $log.TimeCreated
        $message = $log.message
        "Hostname=$ENV:COMPUTERNAME;QueryDate=$querydate;QueryType=SystemCrash;Crash Reboot Date=$rebootDate;Message=$message"
    }
} 
#
Function get-FailedLogins {
    $fails = Get-EventLog -LogName Security -InstanceId 4625 -Newest 1000
    $querydate = (Get-Date -Format s)
    foreach ($log in $fails) {
        if ($log.timeGenerated) {
            $attemptDate = (get-date $log.TimeGenerated -Format s)
            $logmessage = $log.Message -split "`r`n" 
            $subjectAccountName = ($logmessage | sls "Account Name")[0].Line.Split('')[4]
            $FailedAccountName = ($logmessage | sls "Account Name")[1].Line.Split('')[4]
            if (-not($FailedAccountName.Replace("$","") -like $ENV:COMPUTERNAME)){ # skip computer accounts
                $logontypenumber = ($logmessage | sls "Logon Type:").Line.Split('')[4]
                switch($logontypenumber) {
                    ...
                    5 {$LogonType = "Service"}
                    7 {$LogonType = "Unlock"}
                    8 {$LogonType = "Cleartext Network"}
                    9 {$LogonType = "New Credential based"}
                    10 {$LogonType = "Remote Interactive"}
                    11 {$LogonType = "Cached Interactive"}
                    default {$LogonType = "Unknown"}
                }
                $sourceIP = ($logmessage | sls "Source Network Address:").Line.Split()[4]
                $failReason = ($logmessage | sls "Status:")[0].Line.Split()[4]
                "Hostname=$ENV:COMPUTERNAME;QueryDate=$querydate;QueryType=FailedLogon;Failed Attempt Date=$attemptDate;Subject Account Name=$subjectAccountName;Failed Account Name=$FailedAccountName;Logon Type=$LogonType;IP=$sourceIP;Failure Code=$failReason"
            }
        }
    }
}
...
Function Get-SuccessfulLogins {
    $logins = Get-EventLog -LogName Security -InstanceId 4624 -Newest 1000
    $querydate = (Get-Date -Format s)
    foreach ($log in $logins) {
        if ($log.timeGenerated) {
            ...
            $logmessage = $log.Message -split "`r`n" 
            $subjectAccountName = ($logmessage | sls "Account Name")[0].Line.Split('')[4]
            $SuccessAccountName = ($logmessage | sls "Account Name")[1].Line.Split('')[4]
            if (-not($SuccessAccountName.Replace("$","") -like $ENV:COMPUTERNAME)){ # skip computer accounts
                $logontypenumber = ($logmessage | sls "Logon Type:").Line.Split('')[4]
                switch($logontypenumber) {
                    2 {$LogonType = "Interactive"}
                    3 {$LogonType = "Network"}
                ...
                $sourceIP = ($logmessage | sls "Source Network Address:").Line.Split()[4]
                $LogonID = ($logmessage | sls "Logon ID:")[0].Line.Split()[4]
                "Hostname=$ENV:COMPUTERNAME;QueryDate=$querydate;QueryType=SuccessfulLogon;Successful Login Date=$attemptDate;Subject Account Name=$subjectAccountName;Successful Account Name=$SuccessAccountName;Logon Type=$LogonType;IP=$sourceIP;Logon ID=$LogonID"
            }
        }
    }
}
...
Function Check-SCCM {
    $sccm = gwmi SCCM_Image_Build
    $querydate = (Get-Date -Format s)
    if ($sccm.TSBuild){
        "Hostname=$env:COMPUTERNAME;QueryDate=$querydate;QueryType=SCCMCheck;SCCM Build=$($sccm.TSBuild);SCCM Version=$($sccm.TSVer)"
    } else {
        "!!! Alert !!! No SCCM installed on $env:COMPUTERNAME"
    }
}

Function Find-HiddenPowerShell {
    $procs = Get-Process | % {if ($_.Modules -like "*automation.ni*") {$_}}
    foreach ($proc in $procs){
        if ((-not $proc.ProcessName.contains("powershell")) -and (-not $proc.ProcessName.contains("wsmprovhost"))){
            "!!! Alert !!! Hidden PowerShell Modules found on $ENV:COMPUTERNAME"
            $querydate = (Get-Date -Format s)
            "HostName=$ENV:COMPUTERNAME;QueryDate=$querydate;QueryType=HiddenPowerShell;ProcessName=$($proc.ProcessName);PID=$($proc.Id)"
        }
    }
}

Function Check-InjectedModules {
    #TODO get a list of distinct modules loaded into every process and then check to see if that module is on disk
    # if it is on disk then get a hash of the module and check for a signature
    # if the module is not on disk create an alert
}

Function Check-Autoruns {
    # run the Get-PSAutorun without the drivers, services, and WMI
    $autoruns = Get-PSAutorun -BootExecute -Codecs -AppinitDLLs -ExplorerAddons -SidebarGadgets -ImageHijacks -InternetExplorerAddons -KnownDLLs -Logon -Winsock -OfficeAddins -PrintMonitorDLLs -LSAsecurityProviders -Winlogon -ShowFileHash -ErrorAction SilentlyContinue
    $querydate = (Get-Date -Format s)
    foreach ($entry in $autoruns){
        "HostName=$ENV:COMPUTERNAME;QueryDate=$querydate;Querytype=Autoruns;Category=$($entry.Category);Path=$($entry.Path);" `
        + "Item=$($entry.Item);Value=$($entry.Value);ImagePath=$($entry.ImagePath);LastWriteTime=$($entry.LastWriteTime);"`
        + "Version=$($entry.Version);MD5=$($entry.MD5)"
    }
}


Get-HostInfo -ErrorAction SilentlyContinue#
Check-SCCM -ErrorAction SilentlyContinue
Get-NetworkInfo -ErrorAction SilentlyContinue #
Find-HiddenPowerShell -ErrorAction SilentlyContinue  #
Get-WMIPersist  -ErrorAction SilentlyContinue#
Get-Processes -ErrorAction SilentlyContinue #
Get-Services -ErrorAction SilentlyContinue #
Get-InstalledPrograms  -ErrorAction SilentlyContinue#
Get-NetworkStatistics -ErrorAction SilentlyContinue #
Get-ScheduledTasks -ErrorAction SilentlyContinue #
Get-LoggedInUsers
get-FailedLogins
Get-SuccessfulLogins
get-DirtyBootTimes -ErrorAction SilentlyContinue #
